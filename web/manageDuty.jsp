<%-- 
    Document   : manageDuty
    Created on : 29 Mar, 2023, 2:46:53 PM
    Author     : ELCOT
--%>
...
<%
       ...
       Connection con=null;
    {
        try
		{
			...
			con=DriverManager.getConnection("jdbc:mysql://localhost:3306/abilashproject","root","root");
                        ...
                        PreparedStatement ps=con.prepareStatement("Select * from duty_request where date between (?) and (?)");
    ...
    %>
            ...
            try
            {
                ...
                ResultSet rs=st.executeQuery("select * from duty_request where status='pending'");//"select * from duty_request where status='Approved'"
                while(rs.next())
                {
            %>
            <td><%=rs.getString(1)%></td>
            ...
            <td><%=rs.getString(8)%></td>
            <td><%=rs.getString(9)%></td>
            <td><button style="background-color: green;border-radius:border-radius:5px  "><a href="manageDutyApprove.jsp?id=<%=rs.getString(1)%>" style="font-size:15px;background-size:27px ;background-color: green;border-radius:5px ;color: white" > Approve </button></td>
            <td><button style="background-color: red;border-radius:border-radius:5px  "><a href="manageDutyCancel.jsp?id=<%=rs.getString(1)%>" style="font-size:15px;background-size:27px ;background-color: red;border-radius:5px ;color: white" > Cancel </button></td>
        </tr>
        <%
            }
            }
